...
import os, json
from xlutils.copy import copy   # Utilities for working with Excel files that require both xlrd and xlwt.
import string
...
def writeRouge(row, col, value, sheet):
    """
    在原文所在文件的第二个sheet写入相似度数据，
    每调用一次写入一个句子与一个PIO信息的ROUGE矩阵
...
def excludeStopWords(str_input):
    """
    返回字符串str去除stop words 的结果
    :param str_input:
    :return:
    """
    str_input = " ".join([word for word in str_input.translate(str_input.maketrans('', '', string.punctuation)).split()
                        if word not in stopwords.words('english')])
    if not str_input.strip():
        str_input = str_input.join('*')
    return str_input
